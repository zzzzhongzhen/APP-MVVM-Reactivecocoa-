...
////            UITableViewCell *cell = [dataSource tableView:self cellForRowAtIndexPath:[NSIndexPath indexPathForRow:j inSection:i]];
////            [cellsArray addObject:cell.contentView];
////        }
//    }
...
@interface JKSkeletonLoader ()

@property (nonatomic, strong) NSArray *hasAddAnimateViewsArray;
@end

@implementation JKSkeletonLoader
-(void)addLoaderToTargetView:(UIView <ListLoadable>*)listView{
    [self removeLoaderFromTargetView:listView];
    _hasAddAnimateViewsArray = [listView jk_visibleContentViews];
    [self addLoaderToViews:_hasAddAnimateViewsArray];
}
-(void)removeLoaderFromTargetView:(UIView <ListLoadable>*)listView{
//    NSArray *array = [listView jk_visibleContentViews];
    [self removeLoaderFromViews:_hasAddAnimateViewsArray];
    _hasAddAnimateViewsArray = [NSArray array];
}
...
-(void)addLoaderToViews:(NSArray <UIView *>*)views{
    ...
    [views enumerateObjectsUsingBlock:^(UIView * _Nonnull subview, NSUInteger idx, BOOL * _Nonnull stop) {
        if (subview.subviews.count) {
            for (UIView * _Nonnull zsubview in subview.subviews) {
                [zsubview ld_addLoader];
            }
        }else{
