...
- (void)adaptScreenWidthWithType:(AdaptScreenWidthType)type
    ...
    if (![self isExceptViewClassWithClassArray:exceptViews]) {
        ...
        // 是否要对约束进行等比例
        BOOL adaptConstraint = ((type & AdaptScreenWidthTypeConstraint) || type == AdaptScreenWidthTypeAll) && ![self isKindOfClass:ALLineView.class];
        ...
        if (adaptConstraint) {
            [self.constraints enumerateObjectsUsingBlock:^(__kindof NSLayoutConstraint * _Nonnull subConstraint, NSUInteger idx, BOOL * _Nonnull stop) {
                if (subConstraint.firstAttribute == NSLayoutAttributeLeading || subConstraint.firstAttribute == NSLayoutAttributeTrailing) {
        ...
        if (adaptFontSize) {
            ...
            if ([self isKindOfClass:[UILabel class]] && ![self isKindOfClass:NSClassFromString(@"UIButtonLabel")]) {
                ...
                labelSelf.font = [UIFont fontWithName:labelSelf.font.fontName size:AdaptW(labelSelf.font.pointSize)];
            }
            else if ([self isKindOfClass:[UITextField class]]) {
                UITextField *textFieldSelf = (UITextField *)self;
                textFieldSelf.font = [UIFont fontWithName:textFieldSelf.font.fontName size:AdaptW(textFieldSelf.font.pointSize)];
            }
            else  if ([self isKindOfClass:[UIButton class]]) {
                UIButton *buttonSelf = (UIButton *)self;
                buttonSelf.titleLabel.font = [UIFont fontWithName:buttonSelf.titleLabel.font.fontName size:AdaptW(buttonSelf.titleLabel.font.pointSize)];
            }
            else  if ([self isKindOfClass:[UITextView class]]) {
                UITextView *textViewSelf = (UITextView *)self;
                textViewSelf.font = [UIFont fontWithName:textViewSelf.font.fontName size:AdaptW(textViewSelf.font.pointSize)];
            }
        }
